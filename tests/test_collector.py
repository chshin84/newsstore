...
def test_collect_once_stores_items_and_rescan_is_idempotent(store):
...
def test_collect_once_isolates_feed_failure(fsclient):
    ...
    bad = FeedConfig(feed_id="bad", url="https://e/b.rss", source="S")
    good = FeedConfig(feed_id="good", url="https://e/g.rss", source="S")
    store = FlakyStore(fsclient)
    client = httpx.Client(transport=httpx.MockTransport(lambda r: httpx.Response(200, content=RSS)))
    s = collect_once(client, store, [bad, good], now=NOW)
...
def test_collect_once_records_feed_health(store):
    ...
    feed = FeedConfig(feed_id="h1", url="https://e/g.rss", source="S")
...
def test_collect_once_fills_hankyung_body(monkeypatch, store):
    ...
    collect_once(client, store, feeds)
