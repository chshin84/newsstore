...
- collector 잡은 `GEMINI_API_KEY`(백엔드 전용 비밀)를 **Secret Manager**(`gemini-api-key`)로 주입한다(임베딩 패스). prices·stocks 잡에는 넣지 않는다.
...
| Cloud Run Job | `newsstore-stocks` — 팩터·펀더멘털 수집(`run_factors`, secret `fmp-api-key`) |
...
| Cloud Scheduler | `newsstore-prices-hourly` (`*/15 * * * *`) — 가격 5분봉 |
| Cloud Scheduler | `newsstore-stocks-daily` (`0 7 * * *`) — 팩터·펀더멘털 (매일 07:00) |
| Secret Manager | `fmp-api-key` (prices·stocks Job에 `--update-secrets`로 주입; SA에 secretAccessor) |
...
- **노출 범위는 60일 버퍼뿐** — 깊은 아카이브(10년 EOD·1년 5분봉)는 로컬 SQLite(일회성 백필) 몫이라 Firestore 밖. **구글 auth·허용목록 불요**(공개 모델). 배포는 `firebase deploy --only firestore:rules,hosting`.
...
gcloud run jobs execute newsstore-stocks --region=asia-northeast3 --wait
