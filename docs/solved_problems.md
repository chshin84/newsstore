...
- **office gcloud 인증 위치**: 호스트 gcloud가 아니라 **docker 볼륨 `gcloud-cfg`**에 상주(`deploy-office.ps1`가 거기에 저장). 모든 호출은 `/work/ePrism-SSL-ROOT-CA.crt`를 컨테이너에 cp+`update-ca-certificates` 후 동작 — **`docker run`에 `-v "D:/projects/newsstore":/work` 마운트를 빼먹으면 CA 못 심어 SSL 실패**(빈 결과·가짜 0이 나옴, 데이터 없음으로 오인 금지). **사내 CA를 PC에 정식 등록해도 이 우회는 없앨 수 없다** — 아래 2026-08-13 항목 참조.
...
- **청크 루프는 step과 슬라이스 크기를 같이 봐라** — `for i in range(0, n, STEP): chunk = entries[i:i+SIZE]`에서 STEP≠SIZE면 겹치거나(STEP<SIZE) 건너뛴다(STEP>SIZE). 한쪽만 고치는 리뷰·리팩터가 흔한 원인.
...
- **회사 머신 피드 프로빙 SSL 전멸** — 호스트 curl은 연결 자체가 000, Docker httpx는 전 URL `CERTIFICATE_VERIFY_FAILED`(ePrism MITM). 이 머신엔 `ePrism-SSL-ROOT-CA.crt`가 없어 이미지에 CA가 안 구워졌고 `APP_ENV=office`도 소용없다. → 프로빙(피드 존재·본문 유무 확인) 목적이면 Docker 안에서 `httpx.Client(verify=False)` 일회성 스크립트로 충분하다. 단 회사 IP 응답이라 403/차단 판정은 비권위다(위 '피드 도달성은 IP별로 다름' 항목과 같은 취지) — 최종 판정은 배포 스모크 로그(feed_state에 기록됨)로 한다. 실수집까지 필요하면 근본 해결은 CA를 내보내 재빌드하는 것이다(위 ePrism gcloud 항목 참고).
...
## 2026-07-19 index.html 죽은 분석 UI 제거 (SPA 리팩터)
- **인라인 `<script type="module">` SPA는 배포 전에 모듈을 추출해 `node --check`하라** — index.html이 dashboard_logic의 `relTime`을 import했는데 index엔 이미 동명 `relTime`(다른 시그니처)이 있어 "Identifier 'relTime' has already been declared" 문법 에러가 났다. HTML은 문법검사가 안 돌아 놓치기 쉽고, 이대로 라이브 배포했다면 모듈 파싱 실패 → 부트 워치독이 뜨며 **뉴스 리더 전체가 죽었을 것**. → `awk`로 `<script type="module">`~`</script>`를 슬라이스해 `node --check`, 그리고 제거한 심볼들을 `grep`으로 댕글링참조 0 확인, 마지막으로 **로컬 정적서버(node http) + Playwright로 실제 두 탭 렌더까지 확인 후 배포**(검증 후 주장 — curl은 정적 HTML만 보고 JS 실행은 못 본다). 이름 충돌은 `relTime as dsRelTime` alias로 해소.
- **큰 죽은-기능 제거는 "살아있는 진입점에서의 도달가능성"으로 판정하라** — 백엔드 분석 파이프라인이 제거돼 스토리·리포트·v2뷰·시세대시보드 UI가 영구 stale이 됐다(1999→554줄 제거). 위험은 오직 **피드와 removed 기능이 공유하는 헬퍼**(esc·txt·relTime·srcColors)를 오삭제해 살아있는 피드를 깨는 것. → 먼저 피드 `load()`/`render()`의 의존 심볼을 확정(피드는 자체 카드렌더러라 스토리 헬퍼 미사용임을 확인)해 "유지 집합"을 못박고, 나머지를 제거한 뒤 댕글링참조 grep으로 검증. 라우팅(showTab/routeFromHash)은 삭제가 아니라 2탭만 남게 **재배선**. 규모가 크고 라이브면 서브에이전트에 위임하되 **diff 리뷰 + 두 탭 브라우저 스모크를 메인이 게이트**한다.
- **Hosting 릴리스는 전체 스냅샷 — 배포목록이 곧 "무엇이 라이브인가"의 SSOT** — operations.md `$deployFiles`가 index.html·config.js 2개만 나열했는데 index가 이제 dashboard_logic.mjs를 import한다. 그 파일이 빠지면 404 → 상태패널·대시보드 둘 다 깨진다(릴리스=빠진 파일은 사라짐). → import 그래프가 바뀌면 배포목록을 함께 갱신하라(문서 드리프트 정정).

## 2026-07-22 save_vectors 청크 겹침 버그 (Firestore 쓰기 최대 5배 증폭)
- **`range(0, len, 50)`인데 슬라이스는 옛 `[i:i+250]`로 남아있어 청크가 겹침** — "768차원 벡터는 250건이면 Firestore 커밋 10MiB 초과 → 50건으로 축소" 주석과 함께 for문의 step만 50으로 바꾸고 슬라이스 크기는 안 고쳐, 같은 항목이 최대 5개 청크에 겹쳐 들어가 매번 중복 커밋됐다(실측: pending=484인데 embedded=1920, pending=264인데 embedded=820 — 로그의 `embedded`가 `pending`보다 크면 이 패턴을 의심). Gemini 임베딩 API 호출 자체는 빠른데(484건 batchEmbedContents 완료까지 ~5초) **Firestore 쓰기 단계만 중복 커밋으로 부풀어 16초**가 걸린 걸 Cloud Logging 타임스탬프 대조로 확인했다. 게다가 원래 고치려던 문제(250건 트랜잭션 10MiB 초과 위험)도 매 라운드 첫 몇 청크가 여전히 250건이라 실제로는 안 고쳐진 채였다. → 해결: `chunk = entries[i:i+50]`로 슬라이스도 step과 맞춤. 회귀 테스트(`tests/test_firestore_store.py::test_save_vectors_no_duplicate_writes_across_chunk_boundary` — 50건 경계를 넘는 60건 배치에서 반환값이 정확히 60인지 검증) 추가 후 이미지 재빌드·Job 갱신까지 배포. **일반화(핵심 gotchas에도 등재): 청크 루프를 고칠 땐 step과 슬라이스 크기를 항상 같이 봐라.**

## 2026-07-26 문서·코드 드리프트 전수 감사 (ultracode)
- **배포 명령에 절대경로를 박으면 "성공하는 오배포"가 된다** — `operations.md`의 `$deployFiles`가 옆 저장소 `D:\projects\data-only\web\*`를 가리키고 있었다. 그 저장소가 **실재**해서 명령은 오류 없이 끝나고, 라이브 사이트에는 옛 `collector` job_health 키와 5분 신선도를 쓰는 낡은 대시보드가 올라간다(Fail-Loud 무력화). 게다가 Hosting 릴리스는 전체 스냅샷이라 피해가 사이트 전면에 미친다. → **배포 목록을 손으로 유지하지 마라** — `Get-ChildItem web -File`처럼 디렉터리에서 **도출**하고, 스크립트 앞에 실행 위치 전제를 적어라. 경로가 다른 저장소를 가리킬 수 있는 명령은 상대경로가 안전장치다.
- **문서에 손으로 박은 개수·주기는 예외 없이 어긋난다** — 실측 대비 활성 피드 52→84, 네이버 키워드 11→45, 임베딩 상한 500→5000(`DEFAULT_CAP`), 수집 주기 5분→15분으로 전부 낡아 있었고, 문서가 인용한 `poll_minutes` 키는 config에도 코드에도 없었다. → 숫자를 문서에 복제하지 말고 **설정 파일이나 상수 이름을 가리켜라**("`config/feeds.yaml`이 SSOT다", "`embed_pass.DEFAULT_CAP`(현재 5000)"). 숫자를 꼭 적어야 하면 상수 이름을 함께 적어 다음 사람이 어디를 고칠지 알게 하라.
- **가드 테스트의 기대 목록이 손으로 유지되면 가드가 초록인 채로 프로덕션만 깨진다** — `test_index_contract.py`의 REQUIRED가 상태 탭 쿼리(`where(source==X).order_by(fetched_at desc)`)를 빠뜨려, 인덱스 미선언이 초록으로 통과했다. 에뮬레이터는 복합 인덱스를 무시하므로 테스트로도 안 잡힌다. 더구나 UI의 소스별 `catch`가 예외를 삼켜 전 행이 "조회 오류"로만 보이는 조용한 실패였다. → **UI가 새 쿼리를 추가하면 인덱스 선언과 가드 REQUIRED를 같이 늘려라.** 예외를 삼키는 per-item catch는 조용한 실패의 온상이니 인덱스 계약과 짝지어 관리하라.
- **멀티에이전트 검증 단계는 관대해질 수 있다 — 파급 큰 주장은 메인이 직접 실측하라** — 조사 79건 중 78건이 검증을 통과했다(반증 1건). 통과율이 이렇게 높으면 검증이 작동한 게 아니라 무뎌졌다고 봐야 한다. 실제로 배포 경로·인덱스 누락·개수 오차 같은 파급 큰 항목은 메인 세션이 `grep`과 `gcloud`로 다시 재보고서야 확신할 수 있었다. → 서브에이전트 검증은 **후보를 걸러주는 장치이지 판정이 아니다.** 수정이 라이브·비용·배포에 닿는 항목은 메인이 한 번 더 실측하라. 특히 인프라 상태(스케줄러 주기, 잡 실재 여부)는 저장소가 아니라 `gcloud`가 SSOT다.

## 2026-07-26 서버 점검에서 드러난 잔재 정리 + fail-loud 가드가 스스로 죽은 버그
- **fail-loud 가드가 자기 입력에서 먼저 죽으면 원인 진단이 통째로 사라진다** — `parse_feed`의 "이건 피드가 아니다" 판정이 `fp.version`을 **속성으로** 읽는데, 본문이 0바이트인 200 응답에서는 feedparser 결과에 `version` 키가 **아예 없다**(`bozo=False`, `entries=[]`). 그래서 `FeedParseError`(차단·오류 페이지) 대신 `AttributeError`가 나고, collector의 포괄 `except`가 이를 "parse/store error"로 뭉개 원인을 지웠다. 프로덕션에서 07-25 새벽 15분마다 재발했는데 잡은 `exit 0`이라 스케줄러는 초록이었다. → **가드가 읽는 값은 반드시 `.get()`으로 읽어라.** 없는 키를 '값 없음'으로 다루면 가드가 의도대로 분류하고, 속성 접근은 가드 자신을 죽인다. 회귀 테스트는 실제 크래시 입력(`parse_feed(b"")`)으로 박았다.
- **기능을 지워도 잔재는 코드가 아니라 인프라에 남는다** — 저장소에서 엔트리포인트를 지우고 잡을 통합한 뒤에도 클라우드에는 레거시 Cloud Run 잡 3개, PAUSED 스케줄러 3개, 선언에 없는 복합 인덱스 5개, 사라진 컬렉션(`prices`·`price_bars`)을 가리키는 TTL 정책 2개가 남아 있었다. 저장소만 보면 전부 깨끗해 보인다. → **기능 제거의 완료 판정은 `gcloud`로 하라.** 특히 인덱스는 `firestore.indexes.json`과 배포 목록을 대조해야 잉여를 볼 수 있다 — 계약 테스트는 선언 누락만 잡고 **배포에만 있는 잉여는 구조적으로 못 잡는다**(에뮬레이터가 복합 인덱스를 무시하므로 테스트로 메울 수도 없다).
- **삭제해도 되는지는 데이터 유무로 갈라라** — 잔재 정리에서 파괴적인 것은 오직 데이터다. 컬렉션 목록을 먼저 뽑아(`documents:listCollectionIds`) 남은 컬렉션이 다섯 개뿐임을 확인하자 "가격 데이터를 지울 것인가"라는 어려운 질문 자체가 사라졌다 — 지울 데이터가 없었고 남은 것은 설정뿐이었다. → **정리 전에 무엇이 실재하는지부터 세라.** 그러면 대부분의 잔재는 가역적인 설정이고, 사용자 결정이 필요한 항목만 남는다.
- **서브에이전트에 주는 사실표에서 빠뜨린 항목이 그대로 오판이 된다** — 감사 에이전트에게 배포 사실을 주면서 **이미지 push 시각을 빼먹었더니**, "task_type 계약이 커밋만 되고 배포되지 않아 프로덕션 벡터에 필드가 없다"는 major 발견이 나왔다. 실제로는 그날 이미지가 이미 올라갔고 벡터 19만 6천 건 전부가 필드를 갖고 있었다. 적대 검증자도 같은 사실표를 봤으니 이를 반증할 수 없었다. → **주입한 사실표의 공백은 검증으로 메워지지 않는다.** 배포 관련 감사에는 이미지 digest와 push 시각을 반드시 함께 넣고, 결론이 배포 여부에 걸린 발견은 메인이 런타임 데이터로 다시 재라.

## 2026-07-26 임베딩 task_type 계약 공백 + 전량 재임베딩
- **`task_type` 미지정은 조용한 계약 공백이다** — 임베딩 호출이 `output_dimensionality`만 넘기고 `task_type`을 비워두면 API 기본값이 쓰여 값은 정상으로 보인다. 그러나 계약 문서가 "다운스트림도 같은 규칙으로 쿼리를 임베딩해야 한다"고 하면서 규칙 목록에 모델·차원만 있고 task_type이 없으면, 다운스트림은 좌표계를 맞출 근거가 없다. 실측: 같은 문장을 타입만 바꿔 임베딩하면 코사인 0.83~0.87로 **분명히 다른 벡터**가 나온다(1.0이 아니다). → **임베딩 계약에는 모델·차원·task_type을 함께 적어라.** 참고로 API가 받는 값은 용도 축(RETRIEVAL_DOCUMENT/QUERY·SEMANTIC_SIMILARITY·CLUSTERING·CLASSIFICATION·QUESTION_ANSWERING·FACT_VERIFICATION·CODE_RETRIEVAL_QUERY)이며 **도메인 축(FINANCE 등)은 400으로 거부된다** — 금융 특화는 임베딩 타입이 아니라 다운스트림 메타데이터(`asset_hint`) 필터로 풀 문제다.
- **멱등 가드가 계약 변경을 영영 막는다** — 백필의 `mark_pending`이 "벡터 있으면 skip"이라, 모델이나 task_type을 바꿔도 재임베딩 대상이 하나도 안 잡혔다. → **계약을 가르는 값(모델·task_type)을 산출물 문서에 함께 저장하고, 멱등 판정을 '존재'가 아니라 '현행 계약과 일치'로 바꿔라.** 그래야 계약 변경이 곧 재작업 대상 선정이 된다. 판정 시 `get_all(field_paths=[...])`로 계약 필드만 투영하라 — 19만 건에서 768차원 본문까지 받으면 페이로드가 GB 단위다.
- **전량 재작업은 Cloud Run 잡보다 로컬 도커가 낫다** — 잡은 task-timeout 3600초(실측 소요 60분과 아슬아슬)에 메모리 512Mi(19만 스냅샷 materialize 시 OOM 위험)에 `maxRetries: 0`이었다. 로컬은 이 셋이 **패치가 아니라 아예 없고**, 로그를 보며 중단할 수 있다. 임베딩 병목이 Gemini 서버측 처리량이라 로컬이어도 느려지지 않았다(40라운드 60분). 준비물은 ADC 마운트(`%APPDATA%\gcloud` → 컨테이너)와 API 키인데, **키는 `.env`에 쓰지 말고 `gcloud secrets versions access`로 셸 환경변수에 담아 `-e KEY`(값 없이)로 통과시켜라** — 디스크에도 명령줄에도 남지 않는다.
- **"전량 재작업이 끝났는가"는 min(타임스탬프)로 도출하라** — Firestore는 "필드 없음"을 쿼리할 수 없어 잔여 stale을 직접 셀 수 없다. 대신 컬렉션 전체에서 `embedded_at` **오름차순 1건**을 읽어 그 값이 작업 시작 시각 이후인지 보면 된다. 이후면 낡은 문서가 0건임이 증명된다(read 1회).
- **task_type은 이름으로 고르지 말고 재서 골라라 — 직관과 반대였다** — 용도가 중복 접기와 군집이라 `CLUSTERING`이나 `SEMANTIC_SIMILARITY`가 맞아 보였는데, 실 저장 기사로 재보니 `RETRIEVAL_DOCUMENT`가 이겼다. 중복 쌍(다른 매체가 쓴 같은 사건)과 무관 쌍을 가르는 AUC가 각각 **1.000(마진 0.256) / 1.000(0.180) / 0.967(0.176)**로, `CLUSTERING`은 완벽 분리에 **실패**했다. 원인은 압축이다 — 뒤 둘은 유사도 값을 잘 맞추도록 훈련돼 무관한 기사 쌍조차 0.77로 띄우는 반면, `RETRIEVAL_DOCUMENT`는 거대 코퍼스에서 문서를 구별하도록 훈련돼 공간이 넓게 퍼진다. → **임계값으로 접고 가르는 용도에는 압축된 공간보다 퍼진 공간이 유리하다.** 과병합이 알려진 실패 모드인 도메인에서는 마진이 곧 안전 여유다. 그리고 **타입 간 비교는 코사인 평균이 아니라 척도 무관 지표(AUC·순위 정확도)로 하라** — 타입마다 유사도 스케일 자체가 달라 평균 비교는 오도한다.

## 2026-08-13 TTL 1년 확장 + 사내 CA 등록으로도 안 풀리는 gcloud
- **사내 루트 CA를 PC에 정식 등록해도 gcloud는 여전히 막힌다** — `REQUESTS_CA_BUNDLE`·`SSL_CERT_FILE`·`NODE_EXTRA_CA_CERTS`가 모두 사내 번들을 가리키는데도 gcloud 572가 `CERTIFICATE_VERIFY_FAILED: Missing Authority Key Identifier`로 죽었다. gcloud의 `core/custom_ca_certs_file`을 그 번들로 바꿔도 결과가 같았다(두 번들 모두 실측). 원인은 신뢰가 아니라 **형식 거부**다 — urllib3 v2가 엄격 모드로 AKI 확장을 요구하는데 ePrism 인증서에 그것이 없다. → **CA를 아무리 더해도 이 검사는 통과할 수 없으니 `deploy-office.ps1`의 옛 gcloud 402 컨테이너 우회를 폐기하지 마라.** "인증서 깔았으니 이제 될 것"이라는 가정을 버리고 `gcloud info --run-diagnostics`로 먼저 재라. 반대로 파이썬·Node는 그 번들로 정상 동작하므로 우회가 필요한 대상은 gcloud뿐이다.
- **TTL 상수를 바꿔도 기존 문서는 옛 기간에 사라진다** — Firestore TTL 정책은 기간을 모른다. 기간은 각 문서의 `expire_at` 값에 이미 박혀 있어서, `_TTL`을 바꾸고 이미지를 재배포해도 **그 뒤에 수집된 문서만** 새 기간을 받는다. 정책을 다시 걸어도 소용없다. → **보존 기간 변경은 상수 한 줄이 아니라 상수 + 전수 백필이다.** 백필은 `items`와 `item_vectors`를 **한 배치에서 함께** 고쳐라 — 벡터가 원본의 `expire_at`을 미러링하는 계약이라 한쪽만 늘리면 벡터가 먼저 사라진다. 배치가 원자적이면 '항목은 맞고 벡터는 틀린' 중간 상태가 없어 이미 맞은 문서를 건너뛰는 최적화가 안전해진다.
- **기간을 더하는 백필은 첫 실행이 정답이라 테스트를 통과한다** — 60일을 1년으로 늘리며 `expire_at += 305일`로 짜면 한 번 돌렸을 때 값이 정확히 맞는다(60+305=365). 실제로 그 구현을 심어 보니 결과 검증 테스트가 **전부 초록**이었고 오직 멱등성 테스트만 FAIL했다. 두 번 돌리면 670일이 되고 중간에 끊겨 재실행하면 일부만 밀려 값이 뒤섞이는데, 단일 실행 테스트로는 그것이 안 보인다. → **백필은 더하지 말고 원천에서 다시 계산하라**(`fetched_at + _TTL`). 그리고 재실행 가능한 스크립트에는 **연속 두 번 돌려 두 번째가 아무것도 고치지 않는지 보는 테스트**를 반드시 넣어라 — 이 결함을 잡는 유일한 그물이다.
