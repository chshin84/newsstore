...
> - **피드/소스 확장(§9)은 별도 스펙으로 분리됨** → **`2026-06-28-feed-source-expansion-design.md`가 소스 레이어의 SSOT**다. 소스 관련 작업은 그 문서에서 하라(여기 §9는 역사적 맥락으로만 남김).
> - **메서드·토픽·델타·점수·UI(§3–§8, §10–§13)는 다른 세션들이 적극 탐색 중**이다. 그 세션들의 결과가 SSOT가 된다 — **이 문서를 단독 구현 근거로 삼지 말 것.** 본 문서는 그날의 합의 스냅샷이며, 메서드 부분은 갱신·대체될 수 있다.
> - 즉: **소스 = 분리 스펙 / 메서드 = 진행 중**. 이 마스터 문서를 그대로 plan화하지 말 것.
