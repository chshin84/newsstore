...
- **인리치 = Cloud Run Job#2**(asia-northeast3, **Firestore와 동일 리전 필수** — 병목 해결의 본질). 비용은 §7(서울 Tier 2라 소액 발생). 수집 주기는 5분 유지(1분 상향은 비용만↑, 이득 작음).
...
- ⚠️ **리전 긴장(중요)**: "무료 위해 Tier 1 리전(us-central1)"은 **함정** — 인리치가 Firestore(서울)와 분리되면 쓰기 ~1000건/run이 **cross-region 왕복 → over-internet 병목 부활**(애초에 고치려던 것). I/O 무거운 잡은 **DB와 같은 리전이 핵심**. → **서울 유지 필수**, 무료티어 못 씀.
- **결정 필요(사용자)**: ① **서울 + 5분 주기 + 소액 수용**(≈ $9~18/월, 권장) ② 비용 더 줄이려면 **잡당 Firestore 쓰기 최소화**(batch write·save+mark 통합·재색인 디바운스)로 실행시간↓ → vCPU-s↓. 수집 5분 유지(1분은 비용만↑, YAGNI).
