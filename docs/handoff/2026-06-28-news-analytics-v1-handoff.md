...
- [x] **Firestore 인덱스 2종** — ✅ **둘 다 이미 `firestore.indexes.json`에 선언됨**(추가 불필요). `items`(processed ASC+fetched_at ASC, line 11-18)는 `get_unprocessed` 실쿼리와 일치, `stories`(status ASC+last_seen DESC, line 27-34)는 스펙 그대로. ⚠️ 잔존: **실 Firestore READY 배포 확인**(파일 선언≠배포; `unsolved_problems.md` Phase D "인덱스 배포 확인"). ✅ 정정(2026-06-28): stories 복합인덱스는 **이미 쓰인다** — **웹 UI**가 `where(status==open).order_by(last_seen desc)`로 직접 쿼리(`web/index.html:411`), `tests/test_index_contract.py:13`이 필수로 단언. (Python `get_open_stories`는 `last_seen`을 클라측 필터하지만, 인덱스 필요 여부는 UI 쿼리가 이미 결정 → news-analytics 어댑터 설계와 무관하게 필요·선언됨.) ※ 앞서 채팅 회신에서 "현 코드가 안 쓴다"고 한 것은 오류 — UI 쿼리를 놓침.
