...
## 어디를 볼까
- **스코프 (중요):** newsstore = 수집·저장·호스팅(UI) **+ 인리치/분석을 통합 개발**(클러스터·태깅·임베딩·스토리·risk/impact 등). gray-band 클러스터링 코드는 newsstore `src/enrich/clustering.py`로 **이식**(import 아님). **진행상태·백로그: 메모리 `project-status` · `docs/unsolved_problems.md`** · 분석 설계: `docs/analysis-design.md` · 스키마 계약: `docs/firestore-contract.md` · 메모리 `integration-strategy`·`hybrid-topic-lens-model`.
